...
courses_clean%>%
  group_by(dept)%>%
  summarize(avg_length=str_length(name))%>%
...
```{r}
#R starts counting positions at 1 
stat<-courses_clean%>%
  filter(dept=='STAT')%>%
  mutate(name=str_replace(name, 'Introduction to', ' '))%>%
  mutate(name=str_replace(name, 'Statistical', ' '))%>%
  mutate(start_time = str_sub(time,1,5))%>%
  select(number, name, enrollment, start_time)
...
```{r, fig.width=16}
enrollments%>%
  ggplot(aes(x=dept, y=enrollment))+
  geom_boxplot()
```
