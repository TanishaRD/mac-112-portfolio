...
The reaction time of a subject over multiple days. 
...
-   Besides putting each argument on a different line for readability and storing the results, what changed in the code? names_prefix 
-   How did this impact how the values are recorded in the `day` column? it dropped the prefix 
...
```
We want day to be an integer and we want Subject to be a character (so that they are discrete values). 
...
As people have more days of only 3 hours of sleep, their reaction time increases. 
-   Is this the same for everybody? What differs?
For indivdual 332, this is not the case. Their reaction time peaked around day 6, and then fell. 
\
\
\
\
...
daily_new2=pivot_wider(daily_new, names_from=year, values_from=n, values_fill = 0)

daily_done=daily_new2%>%
  arrange(desc(total))%>%
  head(15)
```
