...
Explain why we should only use `na.omit()` in extreme circumstances. Because we don't know if we may need those rows later, and we might chose to impute the values instead of getting rid of them. 
...
```{r}
# What do these lines produce / what's their difference?
mday(today) # The date (dd)
yday(today)  # This is often called the "Julian day" # day of the year in numbers 
```

```{r}
# What do these lines produce / what's their difference?
wday(today) #The level of the weekday 
wday(today, label = TRUE) #The weekday name 
```
...
today < ymd("2024-02-14") #True 
# We are currently past valentine's day 
```
...
```{r}
Birthdays%>%
  mutate(m=month(date, label=TRUE))%>%
  group_by(year, m)%>%
  ggplot(aes(x=m, y=births))+
    geom_boxplot()
```
