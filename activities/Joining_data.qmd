...
```{r}
 courses_combined%>%
  summarize(median_enroll=median(enroll, na.rm=TRUE))
```
...
grades%>%
  ...
  group_by(sid) %>%
  summarize(mean_gpa_per_student=mean(gp, na.rm = TRUE))
```
...
grades%>%
  left_join(gpa_conversion)%>%
  summarize(med_gpa=median(gp, na.rm=TRUE))
```
...
grades%>%
  ...
  left_join(courses)%>%
  group_by(iid)%>%
  summarize(mean_gpa=mean(gp, na.rm=TRUE))%>%
  arrange(mean_gpa)
```
