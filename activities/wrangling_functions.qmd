...
```{r}
# Creates a new column with just the difference bw repub and dem in 2020
...
elections_small |> 
  mutate(dem_diff_binary = ifelse(dem_pct_20>dem_pct_16, 'True', 'False')) |> 
  head()
```
...
It will only give us counties in WI, where the republican percentage of votes in 2020 was lesser than the democrat percentage of votes, sorted by most to least total votes, only giving us the first 6 rows. 
...
You get rid of some rows before arranging: more computationally efficient. 
...
```{r}
ggplot(mn_wi, aes(fill=state_name, x=dem_diff))+
...
```{r}
#Gives us the median of repub_pct_20 called median_repub
 elections_small |> 
  summarize(median_repub = median(repub_pct_20))
```

```{r}
# Creates 2 columnns, 1 called median repub for median repub_pct_20, and 1 called total votes, that adds all the total votes for every county
 elections_small |> 
...
elections_small%>%
  mutate(dem_votes_20=dem_pct_20*total_votes_20)%>%
  mutate(repub_votes_20=repub_pct_20*total_votes_20)%>%
  group_by(state_name)%>% 
  summarize(total_vote_dem=sum(dem_votes_20), total_vote_repub=sum(repub_votes_20))
